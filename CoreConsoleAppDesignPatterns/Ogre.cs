...
{
    public class Ogre : ICreature
    {
        public Ogre()
        {
            BaseHitPoints = 36 + 6 * Roll(6);
            ArmorClass = 10;
            BaseAttackBonus = Roll(4);
            CurrentHitPoints = BaseHitPoints;
            BaseDamage = 8;
            Name = String.Format("Ogre {0}", this.BaseHitPoints);
            ToHitModifier = 0;
        }
        ...
        {
            Console.WriteLine("{0} Stats - AC {1}   HP {2}  Current HP {3}", this.Name, this.ArmorClass, this.BaseHitPoints, this.CurrentHitPoints);
